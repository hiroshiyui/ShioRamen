...
### Fixed
- (build): added `vendor/mruby/build/host/include` to the C glue include path so `mruby/presym/id.h` (generated by rake) resolves under mruby 4.0.0.

### Maintenance
- (vendor): bumped `vendor/mruby` submodule from 3.4.0 to 4.0.0.
- (config): switched `mmproj` projector path to `./models/mmproj-BF16.gguf`.
