...
impl ToolExecutor {
    ...
    fn write_file(&self, args: &Value) -> String {
        let path    = match args["path"].as_str()    { Some(p) => p, None => return "Error: missing 'path'".into() };
        let content = match args["content"].as_str() { Some(c) => c, None => return "Error: missing 'content'".into() };
    ...
    fn list_directory(&self, args: &Value) -> String {
        ...
        let mut lines: Vec<String> = entries
            ...
            .collect();
        lines.sort();
        if lines.is_empty() { "(empty)".into() } else { lines.join("\n") }
    }
...
fn short_display(args: &Value) -> String {
    let Value::Object(map) = args else { return args.to_string() };
    map.iter()
        .map(|(k, v)| {
            let val = match v {
                Value::String(s) => {
...
mod tests {
    ...
    fn executor(confirm_writes: bool, confirm_shell: bool) -> ToolExecutor {
        ToolExecutor { confirm_writes, confirm_shell }
    }
    ...
    fn write_file_creates_file() {
        ...
        assert!(result.contains("written") || result.contains("bytes"), "{result}");
